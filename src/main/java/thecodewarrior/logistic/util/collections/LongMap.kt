 ...
 */
class LongMap<T: Any>(size: Int,
                 /** Fill factor, must be between (0 and 1)  */
                 private val m_fillFactor: Float) {

    constructor() : this(16, 0.5f)

    val keySet: Iterable<Long> by lazy {
        val map = this
        object : Iterable<Long> {
            override fun iterator(): Iterator<Long> {
                return object: Iterator<Long> {
                    var nextCache = FREE_KEY
                    var first = true
                    var keyIndex = 0

                    override fun hasNext(): Boolean {
                        if(first && map.m_hasFreeKey)
                            return true
                        else
                            return nextCache != FREE_KEY
                    }

                    override fun next(): Long {
                        val toReturn = if(first) FREE_KEY else nextCache
                        first = false

                        var nextIteration = FREE_KEY

                        // search for the next non-free key
                        while(keyIndex < map.m_keys.size && nextIteration == FREE_KEY) {
                            nextIteration = map.m_keys[keyIndex]
                        }

                        nextCache = nextIteration
    ...
    /** Values  */
    private var m_values: Array<Any?>

    /** Do we have 'free' key in the map?  */
    private var m_hasFreeKey: Boolean = false
    /** Value of 'free' key  */
    private var m_freeValue: Any? = null
    ...
    init {
        ...
        m_values = arrayOfNulls<Any?>(capacity)
        m_threshold = (capacity * m_fillFactor).toInt()
    }

    operator fun get(key: Long): T? {
        if (key == FREE_KEY)
            return (if (m_hasFreeKey) m_freeValue else NO_VALUE) as T?

        val idx = getReadIndex(key)
        return (if (idx != -1) m_values[idx] else NO_VALUE) as T?
    }

    operator fun set(key: Long, value: T?) { this.put(key, value) }
    ...
    private fun rehash(newCapacity: Int) {
        ...
        m_values = arrayOfNulls<Any?>(newCapacity)
        m_size = if (m_hasFreeKey) 1 else 0

        var i = oldCapacity
        while (i-- > 0) {
            if (oldKeys[i] != FREE_KEY)
                put(oldKeys[i], oldValues[i] as T?)
        }
    }
